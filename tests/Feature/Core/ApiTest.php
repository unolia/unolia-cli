...
it('reads a versioned path', function () {
    $result = cli()
...
it('accepts every spelling of the same path', function () {
    foreach (['v2/teams', '/v2/teams', '/api/v2/teams', 'teams', 'https://app.unolia.com/api/v2/teams'] as $endpoint) {
        $result = cli()
            ->withApi(api()->on('GET', 'v2/teams', fixture('teams.json')))
            ->run('api', $endpoint);
...
it('passes a v1 path through as it is', function () {
    $result = cli()
        ->withApi(api()->on('GET', 'v1/domains', ['data' => []]))
        ->run('api', 'v1/domains');
...
it('refuses a URL on another host', function () {
    $result = cli()->run('api', 'https://example.com/api/v2/teams');
...
it('turns fields into query parameters on a GET', function () {
    $cli = cli()->withApi(api()->on('GET', 'v2/websites?project=12', fixture('websites.json')));

    $cli->run('api', 'v2/websites', '-X', 'GET', '-f', 'project=12');

    expect($cli->api()->lastCall()['query'])->toBe(['project' => '12']);
});

it('sends typed fields as a JSON body', function () {
    $cli = cli()->withApi(api()->on('POST', 'v2/websites/118/deployments', fixture('deployment-create-201.json'), 201));

    $result = $cli->run('api', 'v2/websites/118/deployments', '-X', 'POST', '-F', 'dry_run=false', '-F', 'count=3');
...
it('posts by default when fields are given', function () {
    $cli = cli()->withApi(api()->on('POST', 'v2/issues/abc/ignore', ['data' => []]));

    $cli->run('api', 'v2/issues/abc/ignore', '-F', 'dry_run=false');
...
it('reads a body from stdin', function () {
    ...
    $result = $cli->run('api', 'v2/websites/118/deployments', '--input=-');
...
it('sends an extra header', function () {
    ...
    $cli->run('api', 'v2/teams', '-H', 'X-Trace: abc');
...
it('maps HTTP errors to exit codes', function () {
    $notFound = cli()
        ...
        ->run('api', 'v2/websites/999');

    expect($notFound->exitCode)->toBe(4);

    $unauthenticated = cli()
        ->withApi(api()->on('GET', 'v2/teams', fixture('error-401.json'), 401))
        ->run('api', 'v2/teams');

    expect($unauthenticated->exitCode)->toBe(3);

    $upgrade = cli()
        ->withApi(api()->on('GET', 'v2/automations', fixture('error-402.json'), 402))
        ->run('api', 'v2/automations', '--json');
...
it('merges pages with --paginate --slurp', function () {
    $page = fn (int $page, int $last, array $rows): array => [
        'data' => $rows,
        'links' => ['next' => $page < $last ? 'https://app.unolia.com/api/v2/teams?page='.($page + 1) : null],
        'meta' => ['current_page' => $page, 'last_page' => $last],
    ];

    $result = cli()
        ->withApi(api()
            ->on('GET', 'v2/teams?page=1', $page(1, 2, [['id' => 1]]))
            ->on('GET', 'v2/teams?page=2', $page(2, 2, [['id' => 2]])))
        ->run('api', 'v2/teams', '--paginate', '--slurp');
