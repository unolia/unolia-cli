...
it('adds a record in zone file order, relative to the project zone, and follows it with --wait', function () {
    $cli = oneZone();
    $cli->api()
        ->on('POST', 'v1/domains/acme.com/records', fixture('record-create-201.json'), 201)
...
it('hands the record back at once in a pipe, with the watch hint', function () {
    $cli = oneZone();
    $cli->api()->on('POST', 'v1/domains/acme.com/records', fixture('record-create-201.json'), 201);
...
it('turns @ into the zone, accepts a full hostname, sends the priority, and picks the zone from the hostname', function () {
    ...
    $cli = cli()->withConfig(['team' => 'acme', 'project' => 12])->withApi(api()
        ...
        ->on('POST', 'v1/domains/acme.dev/records', fixture('record-create-201.json'), 201));
    $cli->run('dns', 'add', 'www.acme.dev.', 'A', '203.0.113.10');

    expect($cli->api()->lastCall()['path'])->toBe('v1/domains/acme.dev/records')
...
it('takes --zone anywhere and the old domain add spelling with the zone first', function () {
    ...
    $old = cli()->withApi(api()->on('POST', 'v1/domains/acme.com/records', fixture('record-create-201.json'), 201));
...
it('set creates the record when there is none of that name and type', function () {
    $cli = cli()->withApi(api()
        ->on('GET', 'v1/domains/acme.com/records?name=blog.acme.com&type=A', fixture('records-none.json'))
        ->on('POST', 'v1/domains/acme.com/records', fixture('record-create-201.json'), 201));
...
it('set replaces the value of the one existing record after asking, and says so when nothing changes', function () {
    $cli = cli()->withApi(api()
        ->on('GET', 'v1/domains/acme.com/records?name=www.acme.com&type=A', fixture('records-www.json'))
    ...
    $refused = cli()->withApi(api()->on('GET', 'v1/domains/acme.com/records?name=www.acme.com&type=A', fixture('records-www.json')))
        ->run('dns', 'set', 'www', 'A', '203.0.113.11', '--zone', 'acme.com');

    expect($refused->exitCode)->toBe(2)
        ->and($refused->stderr)->toContain('Replace it with 203.0.113.11?');

    $same = cli()->withApi(api()->on('GET', 'v1/domains/acme.com/records?name=www.acme.com&type=A', fixture('records-www.json')))
...
it('set refuses when several records share the name and type', function () {
    ...
    $result = cli()->withApi(api()->on('GET', 'v1/domains/acme.com/records', $two))
        ->run('dns', 'set', 'www', 'A', '203.0.113.11', '--zone', 'acme.com');
...
it('says when a name matches no record, or several', function () {
    $none = cli()->withApi(api()->on('GET', 'v1/domains/acme.com/records', fixture('records-none.json')))
        ->run('dns', 'remove', 'nope', '--zone', 'acme.com');

    expect($none->exitCode)->toBe(4)
        ->and($none->stderr)->toContain('no nope record in acme.com');

    $several = cli()->withApi(api()->on('GET', 'v1/domains/acme.com/records', fixture('records.json')))
